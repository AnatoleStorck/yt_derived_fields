...
# The database is saved as coll_line_dict.npy and rec_line_dict.npy for collisional and recombination lines
# respectively. The database contains metadata about the lines, including the ion, atom, upper and lower
# levels, as well as the emissivity grid, which is a RegularGridInterpolator object taking in
# ne [cm**-3] and T [K].
...
import numpy as np
import pyneb as pn
from pyneb.utils.logging import PyNebError
import sys
...
for el in ["o", "n", "s", "ne", "c", "mg"]:
    for i in ["i", "ii", "iii", "iv", "v"]:
        try:
            ...
            print(f"Could not set chianti data for {el}-{i}, using default", file=sys.stderr)
