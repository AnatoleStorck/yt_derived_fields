...
def _initialize_pop2_spectra(ds, combined):
    def pop2_spectra(field, data):
        if combined:
            return pop2_stellar_spectra.get_pop_2_spectrum(data, combined=True)
        else:
...
def _initialize_pop3_spectra(ds, combined):
    def pop3_spectra(field, data):
        if combined:
            return pop3_stellar_spectra.get_pop_3_spectrum(data, combined=True)
        else:
...
def get_stellar_continuum(ds, combined=True):
    if not combined:
        raise NotImplementedError("Non-combined stellar spectra are not yet implemented.")
    stars_derived_fields.create_star_derived_fields(ds)
    _initialize_pop2_spectra(ds, combined)
    _initialize_pop3_spectra(ds, combined)
