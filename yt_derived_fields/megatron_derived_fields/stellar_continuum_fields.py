# An attempt to generate a spectra per cell. The idea is to check the stars in each cell and assign a stellar spectra to each
# In practice, one would want to use this on bins of cells, such as to create an IFU with a regular grid.

# TODO: Add the ability to generate spectra for a given cell (set everything to zero then add the spectra of the stars in that cell)
    ...
    def pop2_spectra(field, data):
        ...
        if Npop2 > 0:
            ...
            for unique_star_cell_id in unique_star_cell_ids:
                gas_pop2_spectra += np.sum([pop2_spec[index] for index in stars_cell_ids[stars_cell_ids == unique_star_cell_id]])
            
        return gas_pop2_spectra
    
    ds.add_field(name=("gas", "pop2_spectra"),
...
def _initialize_pop3_spectra(ds):
    ...
    def pop3_spectra(field, data):
        ...
        if Npop3_alive > 0:
            ...
            for unique_star_cell_id in unique_star_cell_ids:
                gas_pop3_spectra += np.sum([pop3_spec[index] for index in stars_cell_ids[stars_cell_ids == unique_star_cell_id]])
                
        return gas_pop3_spectra
        
    ds.add_field(name=("gas", "pop3_spectra"),
