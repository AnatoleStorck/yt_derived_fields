...
def cooling_H2GP(nH, nH2, Tgas):
    ...
    LDL = (10**(-103.0 + 97.59*logT - 48.05*logT**2 + 
                 10.8*logT**3 - 0.9032*logT**4)) * nH
    mask_LDL = LDL == 0

    # high density limit
    HDLR = ((9.5e-22*T3**3.76)/(1.0 + 0.12*T3**2.1)*np.exp(-(0.13/T3)**3) +
            3.e-24*np.exp(-0.51/T3))  # erg/s
    HDLV = (6.7e-19*np.exp(-5.86/T3) + 
            1.6e-18*np.exp(-11.7/T3))  # erg/s
    HDL = HDLR + HDLV  # erg/s
    mask_HDL = HDL == 0
...
def _initialize_photoheating_heating(ds):
    def _photoheating_heating(field, data):
        ...
        return (data["ramses", "hydro_heating_rate"] -
...
def _initialize_H2_cooling(ds, H2_cooling):
    ...
    u_ds = ds.units # Need RAMSES specific units
